...
/* if choose current_node, how many times current node should be choose */
const int T[3][3] = {
    {1, 0, 1},
    {0,-1, 0},
    {1, 0, 1}
...
/* if choose current_node, then choose left child or not */
const int TL[3][3] = {
    {1, 1, 1},
    {0, 0, 0},
    {0, 0, 0} 
...
/* if choose current_node, then choose right child or not */
const int TR[3][3] = {
    {1, 0, 0},
    {1, 0, 0}, 
    {1, 0, 0},
        ...
        FOR(i1, 0, 3) FOR(i2, 0, 3) {
            FORE(m1, 0, m) {
                ...
                FORE(m2, 0, m) {
                    ...
                    if(i1 == 1 || i2 == 1) {
                        get_max(res[2][m1+m2], sL+sR-(i1==1&&i2==1?s0:0), m1+m2);
                    } else {
                        get_max(res[0][m1+m2], sL+sR, m1+m2);
                    }
                    
                    int curr = sL+sR+T[i1][i2]*s0+TL[i1][i2]*s1+TR[i1][i2]*s2+sp;
                    get_max(res[1][m1+m2+1], curr, m1+m2+1);
                }
            }
        }
